...
- [x] **FOUND-01**: Content layer module (`src/lib/content/`) reads the 5 extracted JSON files (practitioners, locations, services, blog_posts, contact_info) as the single source of truth
- [x] **FOUND-02**: SM brand design tokens (colors, fonts, spacing) defined in Tailwind v4 config — extracted from existing WordPress theme
- [x] **FOUND-03**: HIPAA-safe analytics configured (Plausible or privacy-first solution — no GA4 on medical/service pages)
